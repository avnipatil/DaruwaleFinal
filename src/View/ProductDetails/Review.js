...
import { REVIEW_URL } from '../../endpoint';
...
const Review = React.memo((props) => {
    ...
    const onSubmit = (data) => {
        ...
        axios.post(REVIEW_URL + "/" + props.id, data).then((res => {
            if (res.status === 200) {
                ReviewData.push({
                    "name": data.name,
                    "email": data.email,
                ...
                <div className='col-md-7'>
                    {
                        ReviewData.length === 0 ?
                            ...
                            <div className='text-center'>
                                <i className="fa fa-comments-o" aria-hidden="true" style={{fontSize: '10rem',color:'#0d324d'}}></i>
                                <br />
                                <h3>You are The first to be comment </h3>
                            </div>
                :
                            ...
                            <div className="pb-4 mb-4 border-bottom" key={index}>
                                <div className="d-flex mb-3 justify-content-between">
                                    <div className="d-flex align-items-center me-4 pe-2">
                                        <img className="rounded-circle" src={ReviewProfileImg} width="50" alt="Rafael Marquez" />
                                        <div className="ps-3">
                                            <h6 className="mb-0">{value.name}</h6>
                                            <span className="Review-comment-date text-muted">{value.date}</span>
                                        </div>
                                    </div>
                                    <div>
                                        <div className="star-rating">
                                            {
                                                Array.from(Array(value.rating), (e, i) => {
                ...
                <div className='Review-Comment-Form rounded-3'>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} >
                        ...
                        <div className="form-group mb-3">
                            ...
                            <select className="form-select" required="" id="review-rating" {...register("rating", {
                                required: true,
                            })}>
                            ...
                            </select><span className="error-msg" title="invlid subject">{errors.rating && "Please Choose Rating"}</span>
                        </div>
                        <div className="form-group mb-3">
